...
| Broader attached-property metadata | Experimental | Built-in metadata, validation, runtime application/clearing, completion, hover, semantic tokens, and quick-fix suggestions now cover `Canvas`, `RelativePanel`, `ToolTipService`, `VariableSizedWrapGrid`, and expanded `AutomationProperties` entries. External attached-property owner discovery remains outside v1. |
