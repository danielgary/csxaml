...
- 2026-04-27: Hardened the preview NuGet publish path after NuGet.org rejected `Csxaml.0.1.0-preview.1.snupkg` because the wrapper package carried generator-tool PDBs rather than normal library symbols. `Csxaml` no longer produces a symbol package or packs generator PDBs, `Csxaml.Runtime` remains symbol-packaged, and release package validation now scans every package artifact before both clean-consumer validation and NuGet publishing.
