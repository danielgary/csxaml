...
- 2026-05-01: Resolved the Feature Gallery Fluent-card `ListView` wheel/flicker issue without keeping the diagnostic scaffolding. The temporary UI automation project, native wheel probes, feature-page probe panel, and handwritten wheel probe sample were removed after diagnosis; the retained sample fixes are the DPI-aware manifests for Feature Gallery and Todo plus a passive `FluentCard` surface.
- 2026-05-01: Hardened external-control content projection around explicit content properties. `FluentCard` now exposes a `Child` content property on a plain `Grid`, external-control metadata prefers declared content metadata before inherited panel `Children`, and projection skips inherited non-writable collection properties such as `Grid.ColumnDefinitions` and `Grid.RowDefinitions`.
- 2026-05-01: Closed the default-content reapplication path that caused retained external controls to flicker. Default content properties are no longer emitted or applied as ordinary clearable external properties, so `FluentCard.Child` is not cleared during property application and then reattached during child projection on every retained render.
